...
def compute(stocks, stocks_qty,time_horizon_of_investment= 2):
    ...
    for  stock , stock_qty in zip(stocks,stocks_qty):
        ...
        if stock in ans:
            health_of_stock = ans[stock]['health_of_stock']
            invested_value = ans[stock]['invested_value']
            ...
            health_of_stock = (rsi_factor*(TS.RSI_RATIONALIZED(rsi_days_factor, closing_prices_array)))
            health_of_stock = round(health_of_stock, 2)
            invested_value = stock_qty*closing_prices_array[-1]
            ans[stock] = {"health_of_stock": health_of_stock, "invested_value": invested_value, "time":time.time()}

        send_data.append({"id":stock.split(".",1)[0], "quantity": stock_qty, "health": health_of_stock})
